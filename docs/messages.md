...
## List of Messages
### Version
The version message (`ver`) is used on connection creation, to advertise the type of node. The remote node will respond with its version, and no communication is possible until both peers have exchanged their versions. By default, fcash-base advertises itself as named `fcash-base` with the current version of the `fcash-p2p` package.
